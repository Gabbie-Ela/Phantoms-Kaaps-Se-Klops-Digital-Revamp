...
import com.example.phantoms.utils.DefaultCard.GetDefCard
import com.example.phantoms.utils.Extensions.cardXXGen
import com.example.phantoms.utils.Extensions.toast
...
class ProductDetailsActivity : AppCompatActivity() {
    ...
    private fun setProductData() {
        var fileJson = when (ProductFrom) {
            "Cover" -> "CoverProducts.json"
            "New"   -> "NewProducts.json"
            else    -> "NewProducts.json"
        }

        val jsonFileString = this.let { getJsonData(it, fileJson) }
        val gson = Gson()
        val listCoverType = object : TypeToken<List<Product>>() {}.type
        val coverD: List<Product> = gson.fromJson(jsonFileString, listCoverType)

        // smart image loader (handles res:// and fallback)
        productImage_ProductDetailsPage.loadSmart(this, coverD[productIndex].productImage, R.drawable.bn)

        // Bind text
        productName_ProductDetailsPage.text   = coverD[productIndex].productName
        productPrice_ProductDetailsPage.text  = "R${coverD[productIndex].productPrice}"
        ...
        RatingProductDetails.text             = "${coverD[productIndex].productRating} Rating on this Product."

        // Fields used for cart
